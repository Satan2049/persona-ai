...
### Typical assets (v1.3.3+)

| Platform | Asset | Checksums |
|----------|--------|-----------|
| Windows | NSIS `.exe` installer | `SHA256.txt` (maintainer) |
| Linux | AppImage / `.deb` | `SHA256-linux.txt` (CI on tag) |
| macOS | `.dmg` (Apple Silicon CI) | `SHA256-macos.txt` (CI on tag) |
...
### v1.3.3 — official VirusTotal report

| Asset | SHA256 | Report |
|-------|--------|--------|
| NSIS setup (`Persona AI_1.3.3_x64-setup.exe`) | `f0546756725c726f97d3202a44aa1aebf85125b3a28ac09c50e7bcb3afc6441a` | [View on VirusTotal](https://www.virustotal.com/gui/file/f0546756725c726f97d3202a44aa1aebf85125b3a28ac09c50e7bcb3afc6441a?nocache=1) |

No malicious detections at the time of the scan.

After building any additional Windows assets, run `npm run release:checksums` and upload `SHA256.txt` with the release. Linux/macOS checksums (`SHA256-linux.txt`, `SHA256-macos.txt`) are generated by CI when tag `v1.3.3` points at the 1.3.3 commit.
